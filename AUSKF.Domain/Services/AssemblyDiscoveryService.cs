namespace AUSKF.Domain.Services
{
    ...
    public sealed class AssemblyDiscoveryService : IAssemblyDiscoveryService
    {
        ...
        /// </summary>
        /// <param name="configurationService">The configuration wrapper.</param>
        /// <exception cref="AppDomainUnloadedException">The operation is attempted on an unloaded application domain. </exception>
        public AssemblyDiscoveryService(IConfigurationService configurationService)
        {
            this.AssemblyList = new HashSet<Assembly>();
            this.binDirectory = AppDomain.CurrentDomain.BaseDirectory + (configurationService.AppSettings["DependencyFolder"] ?? "bin");
        }
